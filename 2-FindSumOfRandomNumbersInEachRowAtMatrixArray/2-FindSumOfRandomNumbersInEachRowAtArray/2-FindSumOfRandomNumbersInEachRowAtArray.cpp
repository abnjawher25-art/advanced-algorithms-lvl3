...
void PrintSumOfEachRowAtMatrixArray(short arr[3][3], short rows, short columns) {
    
	cout << "\n\nFollowing are the sum of each row: \n" << endl;
    int sumEachRow = 0;
    for (short position_x = 0; position_x < rows; position_x++) {
        for (short position_y = 0; position_y < columns; position_y++) {
            sumEachRow += arr[position_x][position_y];
        }
        cout << "the sum of row number "<< position_x + 1<<" is =" <<setw(4)<< sumEachRow << endl;
        sumEachRow = 0;
    }
