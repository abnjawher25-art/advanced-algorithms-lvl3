...
// function to prshort the 2D array of random numbers
void PrshortMatrixArrayThatRandomNumbers(short arr[3][3], short rows, short columns) {
...
// function to calculate and prshort the sum of each column in the 2D array
void FillArrayBySumOfColumns(short arr[3][3], short sumArray[3], short rows, short columns) {
    ...
    for (short position_x = 0; position_x < rows; position_x++) {
        for (short position_y = 0; position_y < columns; position_y++) {
            sumEachColumn += arr[position_y][position_x];
        }
        // Store the sum of the current row in the sumArray
        sumArray[position_x] = sumEachColumn;
        sumEachColumn = 0;
    }

}

// function to prshort the sum of each column in the 2D array
void PrshortFullArrayThatSumOfColumns(short arr[3][3], short sumArray[3], short rows, short columns) {

    //prshort array that contains the sum of each columns
...
int main()
{
    ...
    // Prshort the 2D array of random numbers
    PrshortMatrixArrayThatRandomNumbers(arr, 3, 3);

    // Calculate the sum of each column and store it in sumArray
    FillArrayBySumOfColumns(arr, sumArray, 3, 3);

    // Prshort the sum of each column in the 2D array and the corresponding elements in sumArray
    PrshortFullArrayThatSumOfColumns(arr, sumArray, 3, 3);
