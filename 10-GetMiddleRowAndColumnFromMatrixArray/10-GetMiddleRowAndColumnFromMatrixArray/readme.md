أكيد. سأُبقي **نفس القالب والترتيب**، لكن أستبدل المحتوى بالكامل بما يناسب الكود الجديد، خصوصًا أن المشروع لم يعد مجرد توليد مصفوفة؛ أصبح يقوم أيضًا **باستخراج وطباعة الصف الأوسط والعمود الأوسط** من المصفوفة.

وهناك ملاحظة تقنية مهمة: الكود يستخدم `rand()` و`time()` لكنه لا يضمّن `<cstdlib>` و`<ctime>` صراحةً. الأفضل إضافتهما، أما `<random>` فهو موجود لكنه **غير مستخدم فعليًا**.
